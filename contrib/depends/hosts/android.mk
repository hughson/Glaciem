...
# Absolute paths: a bare `llvm-ar` is not on PATH, so autotools configure
# scripts silently fall back to the host's BSD `ar` (which rejects the GNU
# `crsD` archive flags). The NDK toolchain is staged into $(build_prefix).
android_AR=$(build_prefix)/bin/llvm-ar
android_RANLIB=$(build_prefix)/bin/llvm-ranlib
